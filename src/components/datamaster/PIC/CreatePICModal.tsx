...
export default function CreatePICModal() {
    ...
    const [errors, setErrors] = useState({ pic: "" });

    useEffect(() => {
        setPageTitle("Create PIC");
    }, []);
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            setDialogMessage("PIC created successfully.");
            setOpenDialog(true);
            setTimeout(() => {
            ...
            console.error(error);
            setDialogMessage("Error creating PIC.");
            setOpenDialog(true);
        } finally {
            setLoading(false);
        }
                    ...
                    <CardContent>
                        <Box display="flex" flexDirection="column" gap={2} mt={2} ml={4} mr={4}>
                            ...
                            <Box display="flex" justifyContent="flex-end" gap={2} mt={3}>
                                <Button variant="contained" color="error" onClick={handleSubmit} aria-label="Submit PIC">
                                    {loading ? "Submitting..." : "Submit"}
                                </Button>
                                <Button variant="contained" color="warning" onClick={() => navigate("/datamaster/pic")} aria-label="Back to PIC List">
                                    Back
                                </Button>
                            </Box>
                        </Box>
